...
- **主人身份(逐条判定)**:主人 QQ 在 `userData/privacy.json` 的
  `masterQQ` 字段配置(2026-08-17 隐私配置化,安装器/源码不再携带任何
  个人 QQ),任何配置不能改变主人身份;对话窗口直接输入 = 主人本人;带
  【QQ私聊/QQ群聊 · QQ 号】标注的外部消息只有标注主人的才具主人权限;
  【系统通知】= 系统事件。
...
主人 QQ(privacy.json 的 masterQQ 配置)消息 = 最高权限,可直接远程控制
(如"暂停音乐""打开B站")。

### 隐私配置(2026-08-17 起,安装器不携带任何个人 QQ)

主人 QQ / 私聊扩展信任 / 群白名单 / 机器人自身 QQ 一律从
`userData/privacy.json` **运行时读取**(首次启动自动生成空模板),安装器与
源码不再硬编码任何个人身份信息。字段:

```json
{
  "masterQQ": "你的QQ号(唯一主人,必填)",
  "allowed": ["额外可自主回复的联系人QQ"],
  "allowedGroups": ["要监听的群号"],
  "botQQ": "机器人自身QQ(群@检测)"
}
```

- `masterQQ` 为空 = 未配置,QQ 主人相关能力不启用(身份判定恒为"非主人");
- `allowed` / `allowedGroups` / `botQQ` 缺省回填到 Agent 配置,也可在设置/
  `set_napcat_config` 中另行修改。
...
```bash
node scripts/build-release.mjs [--tools=bili,docflow,xxt]  # ① 生成绿色发布目录 release/灵动岛
node scripts/build-installer.mjs                           # ② 生成独立安装器 release/灵动岛安装器(内含 ① 作为安装源)
npx electron installer/main.cjs                            # 开发期直接运行安装向导(不打包,便于预览)
```

- **外部工具按需打包**:`--tools` 逗号分隔选择打包哪些外部工具(bili /
  docflow / xxt;缺省全部)。每个工具目录**整体复制 = 源码 + 编译的 exe
  一并打包**,便于安装后直接修改迭代(如 `tools/bili` 的 Rust 源码 +
  `bili-tool.exe`)。`bili/config`(本机登录态)不随发行。
- **安装时按需勾选**:向导「安装选项」页列出发布包内已打包的工具,默认全选,
  可取消勾选跳过不装(取消后该工具目录不复制到安装目录)。
- **卸载可选删除数据**:`uninstall.cmd`(写入注册表卸载项)运行时交互询问
  「是否同时删除灵动岛的个人数据(聊天记录/记忆/配置)?」——选 Y 删除
  `%APPDATA%\dynamic-island`,选 N 保留数据仅移除程序本体。
- **隐私零携带**:发布产物与安装器均不包含任何个人 QQ/群号(主人 QQ、白名单、
  机器人 QQ 全部走 `userData/privacy.json` 运行时配置)。
